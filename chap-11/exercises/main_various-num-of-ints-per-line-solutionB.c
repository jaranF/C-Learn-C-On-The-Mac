...
#define kDataFileName               "number_data_tab_separated4"
...
#define kScanDigitsToNewlinePattern       "%d"
//|-----------> STRUCTS <-----------|
//|------> TYPEDEFS & ENUMS <-------|
//|--------> FN PROTOTYPES <--------|
...
int main_various_num_of_ints_per_line_solutionB( int argc, const char * argv[] )
{
    int retValue = 0;
...
int GetLineOfNumbers(FILE* stream, int* arrayForlinePtr, long long int perLine) {
    ...
    for (i = 0; i < perLine; i++) {
        amountOfScannedItems = fscanf(stream, kScanDigitsToNewlinePattern, arrayForlinePtr);
